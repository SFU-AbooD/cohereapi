    ...
    if request.method == 'POST':
     ...
     for i in range(len(ans)):
        ...
        if bigger <  ans[i]:
         index = i
         bigger = ans[i]
        print(places[classes[index + 1]])
        return render(request,'chat1.html',context={'data':True,'items':places[classes[index + 1]]})
    return render(request,'chat1.html',context={'data':False})    